...
```{r}
mod_dat <- readRDS('data/mt-data/daily_aggregation_1118.RDS')
setDT(mod_dat)
mod_dat[, daily_activity := sum(daily_boards + daily_alights), keyby = .(GEOID, date_key)]

ggplot(mod_dat, aes(x=log(daily_activity))) +
  geom_histogram()
```
