...
apc <- acs[variable =='tot_pop'][apc, on = 'GEOID']

# this is our response variable for modeling at this point. 
apc <- apc[, .(avg_act_per_capita = mean(avg_activity/estimate), tot_pop = estimate), by = 'GEOID']
...
The idea at this point is to start with some simple linear regressions, then introduce explicit spatial methods, and then introduce time. This document is the starting point. 
...
# bottom code
race_dat1[is.na(avg_act_per_capita) | avg_act_per_capita == 0 | is.infinite(avg_act_per_capita), avg_act_per_capita := 0.01]
